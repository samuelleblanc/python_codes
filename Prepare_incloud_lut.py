...
# Purpose:  
# 
#     Create the input libradtran files for creating a lut of mid clouds to be used in mutliple airborne operational cloud retrievals for use with in-coud data
# 
# Calling Sequence:
# 
#     python Prepare_incloud_lut
#   
# Input:
# 
#     none
...
# History:
# 
#     Written: Samuel LeBlanc, DC8 over Korea, 2016-05-02
#     
...
source = {'wvl_range':[350,1750],
          ...
          'atm_file':'/u/sleblan2/libradtran/libRadtran-2.0-beta/data/atmmod/afglms.dat', #midlatitude summer
          'zenith':True}
...
RL.print_version_details(fp+'KORUS_lut_%s.txt'%vv,vv,geo=geo,
                         aero=aero,cloud=cloud,source=source,albedo=albedo,tau=tau,ref=ref,sza=sza)


# In[71]:

fp_in = os.path.join(fp_rtm,'input','%s_KORUS'%vv)
...
f_list = open(os.path.join(fp,'run','KORUS_list_%s.sh'%vv),'w')
print f_list.name
