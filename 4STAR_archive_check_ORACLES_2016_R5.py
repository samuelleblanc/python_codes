...
for i,d in enumerate(days):
    try:
        print 'Doing day: {}'.format(d)
        fname_aod = fp+'aod_ict/{vi}/4STAR-AOD_P3_{}_{vv}.ict'.format(d,vv='R3',vi='v8')
        tt,th = load_ict(fname_aod,return_header=True)
    except:
        print '*** Problem with day: {} *** Skipping '.format(d)
        days.pop(i)
...
for i,d in enumerate(days):
    ...
    axy.set_position([box.x0, box.y0, box.width * 0.8, box.height])
    ax[1].set_xlabel('UTC [h]')
    plt.savefig(fp+'aod_ict/{vi}/{vv}/{vv}_high_alt_AOD_{}.png'.format(d,vv=vv,vi=vi),dpi=600,transparent=True)
...
for i,d in enumerate(days):
    ...
    ax[0].set_position([box.x0, box.y0, box.width * 0.8, box.height])
    ax[0].legend(frameon=False,loc='center left',bbox_to_anchor=(1.1,-0.2),numpoints=1)
    ax[1].set_color_cycle([plt.cm.gist_ncar(k) for k in np.linspace(0, 1, len(nm[46:-1])+1)])
    for unc in nm[-8:-1]:
        ii = np.where((outaod_RA[i][nm[4]]==0))[0]
        ax[1].plot(outaod_RA[i][nm[0]][ii],outaod_RA[i][unc][ii],'.',label=aod)
    ax[1].set_ylabel('AOD Uncertainty')
...
# ### Plot the difference between R3 and R5 AOD
...
for i,d in enumerate(days):
    ...
    ax.set_title('AOD difference between R3 and R5 for flight {}'.format(d))
    qa = [outaod_RA[i]['qual_flag']==0]
    ax.plot(outaod_RA[i]['Start_UTC'][qa],outaod_RA[i]['AOD0501'][qa]-outaod_R3[i]['AOD0501'][qa],'.')
    
    ax.set_ylabel('Delta AOD (R5-R3) at 501 nm')
    #ax.set_ylim(-0.1,0.1)
    ...
    plt.savefig(fp+'aod_ict/{vv}/{vv}_{}_time_delta_aod_R5vsR3.png'.format(d,vv=vv,vi=vi),dpi=600,transparent=True)
...
for i,d in enumerate(days):
    ...
    qa = [outaod_RA[i]['qual_flag']==0]
    try:
        v = ax.scatter(outaod_RA[i]['Longitude'][qa],outaod_RA[i]['GPS_Alt'][qa],(20*outaod_RA[i][wl[2]][qa])**3.0,
...
for i,d in enumerate(days):
    ...
    v = ax.scatter(outaod_RA[i]['Longitude'][qa],outaod_RA[i]['GPS_Alt'][qa],20,
                   c=outaod_RA[i]['AOD0501'][qa],marker='o',edgecolor='None',vmin=0.0,vmax=1.2)
    try:
        qaf = [(outaod_RA[i]['qual_flag']==0) & (outaod_RA[i]['flag_acaod']==1)]
        print len(qaf)
        v = ax.scatter(outaod_RA[i]['Longitude'][qaf],outaod_RA[i]['GPS_Alt'][qaf],150,
                   c=outaod_RA[i]['AOD0501'][qaf],marker='^',edgecolor='None',vmin=0.0,vmax=1.2)
    except:
        pass
    ax.set_ylabel('Altitude [m]')
    ax.set_ylim(0,7000)
    ...
    points = [ax.scatter([], [], s=s, c='grey',marker=shape[iz],edgecolor='None') for iz,s in enumerate(sizes)]
    plt.legend(points, labels, scatterpoints=1,frameon=False,loc='upper left',bbox_to_anchor=(-1.15,1.25))
    plt.savefig(fp+'aod_ict/{vv}/{vv}_{}_lat_alt_acaod.png'.format(d,vv=vv,vi=vi),dpi=600,transparent=True)


# ### Plot difference in acaod flag R4 to R3
...
for i,d in enumerate(days):
    ...
    try:
        qaf = [(outaod_RA[i]['qual_flag']==0) & (outaod_RA[i]['flag_acaod']==1)]
        print len(qaf)
        v = ax.scatter(outaod_RA[i]['Start_UTC'][qaf],outaod_RA[i]['GPS_Alt'][qaf],60,
                   c=outaod_RA[i]['AOD0501'][qaf],marker='*',edgecolor='None',vmin=0.0,vmax=0.8)
        
        qaf_add = [(outaod_RA[i]['qual_flag']==0) & (outaod_RA[i]['flag_acaod']==1) & ~(outaod_R3[i]['flag_acaod']==1)]
        print len(qaf_add)
        v = ax.scatter(outaod_RA[i]['Start_UTC'][qaf_add],outaod_RA[i]['GPS_Alt'][qaf_add],150,
                   c=outaod_RA[i]['AOD0501'][qaf_add],marker='x',edgecolor='None',vmin=0.0,vmax=0.8)
        
        qaf_rm = [(outaod_RA[i]['qual_flag']==0) & ~(outaod_RA[i]['flag_acaod']==1) & (outaod_R3[i]['flag_acaod']==1)]
        print len(qaf_rm)
        v = ax.scatter(outaod_RA[i]['Start_UTC'][qaf_rm],outaod_RA[i]['GPS_Alt'][qaf_rm],150,
                   c=outaod_RA[i]['AOD0501'][qaf_rm],marker='o',vmin=0.0,vmax=0.8,facecolors='none')
    ...
    sizes = [20,60,150,150]
    shape = ['.','*','x','o']
    labels = ['all AOD','ACAOD','R4 add','R4 del']
    points = [ax.scatter([], [], s=s, c='grey',marker=shape[iz],edgecolor='None') for iz,s in enumerate(sizes)]
    plt.legend(points, labels, scatterpoints=1,frameon=False,loc='upper left',bbox_to_anchor=(-1.15,1.5))
    plt.savefig(fp+'aod_ict/{vv}/{vv}_{}_time_alt_acaod_R4vsR3.png'.format(d,vv=vv,vi=vi),dpi=600,transparent=True)
...
for i,d in enumerate(days):
    plt.figure()
    plt.scatter(outaod_RA[i]['Longitude'],outaod_RA[i]['Latitude'],150,
                       c=outaod_RA[i]['GPS_Alt'],marker='s',edgecolor='None',vmin=0.0,vmax=9000.0)
    plt.colorbar(label='GPS_Alt [m]',extend='max')
    plt.ylabel('Latitude [$^{{\circ}}$]')
    plt.xlabel('Longitude [$^{{\circ}}$]')
    plt.grid()
    plt.title('Flight path {} for flight {}'.format(vv,d))
    plt.savefig(fp+'aod_ict/{vv}/{vv}_{}_lat_lon_alt.png'.format(d,vv=vv,vi=vi),dpi=600,transparent=True)
...
hs.savemat(fp+'/aod_ict/{vv}/all_aod_ict_{vv}_2016.mat'.format(vv=vv),ar)
...
ar = hs.loadmat(fp+'/aod_ict/{vv}/all_aod_ict_{vv}_2016.mat'.format(vv=vv))
