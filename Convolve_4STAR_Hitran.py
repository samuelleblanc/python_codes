...
# Purpose:
# 
#     Load the HITRAN - API and use it to convolve the 4STAR spectra and get the convolved gas cross sections
#     Moslty focused on the measured gas spectra from lab measurements
#     
# Input:
# 
#     None
...
# 
#     Written: Samuel LeBlanc, Santa Cruz, CA, 2022-08-30
#     Modified:
# 
...
build_from_lines = False


# # Load files

# ## Load the 4STAR wavelength and FWHM

# In[6]:


s = sio.loadmat(fp+'4STAR_FWHM_combinedlinelamps_20220507.mat')
...
if ~os.path.exists(fph+'O2-O2_2018b.cia'): 
...
if ~os.path.exists(fph+fo3_223k): 
...
if ~os.path.exists(fph+fo3_293k): 
...
gases = {'no2_vis_220K':{'SourceTables':'NO2','Diluent':{'air':1.0},'Environment':{'T':220.,'p':0.0286}},
         ...
         'hcoh_vis_293K':{'SourceTables':'HCOH','Diluent':{'air':1.0},'Environment':{'T':293.,'p':1.0}},
...
# These are the ones that are created directly from lab measurements - no need to generate atmospehric cross sections
...
# ## Convolve the xs files to 4STAR spectra (in nm)
...
for k in list(gases.keys()):
    if 'wvl' in gases[k].keys():
        ...
        for i,v in enumerate(s['vis_nm'][0]): 
            ...
            gases[k]['xs'][i] = np.trapz(slit*gases[k]['coef'],gases[k]['wvl'])
            pbar.update(1)
        for i,v in enumerate(s['nir_nm'][0]): 
            slit = hapi.SLIT_GAUSSIAN(gases[k]['wvl']-v,s['fwhm_nir'][0,i])
            gases[k]['xs'][1044+i] = np.trapz(slit*gases[k]['coef'],gases[k]['wvl'])
            pbar.update(1)
    else:
        print('** No to gas: '+k)
...
for k in ['o4_vis_296K','o4_nir_296K']:
    for i,v in enumerate(s['vis_nm'][0]): 
        inm_min = np.argmin(abs(gases[k]['wvl']-v-s['fwhm_vis'][0,i]*5.0))
        inm_max = np.argmin(abs(gases[k]['wvl']-v+s['fwhm_vis'][0,i]*5.0))
    ...
    for i,v in enumerate(s['nir_nm'][0]): 
        inm_min = np.argmin(abs(gases[k]['wvl']-v-s['fwhm_nir'][0,i]*5.0))
        inm_max = np.argmin(abs(gases[k]['wvl']-v+s['fwhm_nir'][0,i]*5.0))
...
#sio.savemat(fph+'4STAR_hitran_atm_trace_gases_20220513.mat',gases)
...
# ## Compare to current 4STAR cross sections
...
sio.savemat(fp+'4STAR_20220830_gxs.mat',gxs)
