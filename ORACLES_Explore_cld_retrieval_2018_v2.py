...
#      Modified: Samuel LeBlanc, NASA Ames, Moffett Field, CA, 2019-09-18
#                - modified for use on 2018 data
...
fp = fo+'starzen_2018/v1/'
fp_plot = fo+'plot_2018/'
...
dds = ['20180921','20180922','20180924','20180927','20180930','20181002','20181003','20181005','20181007','20181010','20181012',
...
ar6 = hs.loadmat(fo+'/aod_ict_2018/R1/all_aod_ict_R1_2018.mat')
...
#        rts_mid[i]['ref_fl'] = rts_mid[i]['ref'][rts_mid[i]['fl']]
    ...
    rts_mid[i]['tau_err'] = np.nanmax([rts_05[i]['tau'],rts_10[i]['tau'],rts_65[i]['tau'],rts_80[i]['tau']],axis=0)                          - np.nanmin([rts_low[i]['tau'],rts_10[i]['tau'],rts_65[i]['tau'],rts_80[i]['tau']],axis=0)
    rts_mid[i]['tau_err_fl'] = smooth(rts_mid[i]['tau_err'][rts_10[i]['fl']],6,old=True)
    rts_mid[i]['ref_err'] = np.nanmax([rts_low[i]['ref'],rts_10[i]['ref'],rts_65[i]['ref'],rts_80[i]['ref']],axis=0)                          - np.nanmin([rts_low[i]['ref'],rts_10[i]['ref'],rts_65[i]['ref'],rts_80[i]['ref']],axis=0)
...
hdict = {'PI':'Samuel LeBlanc',
     ...
     'project_info':'ORACLES 2018 deployment (year 3 of 3); September-October 2018; Sao Tome',
     'stipulations':'This is a public release of the ORACLES-2018 data set. We strongly recommend that you consult the PI, both for updates to the data set, and for the proper and most recent interpretation of the data for specific science use.',
     'rev_comments':"""R0: Archival for first public release of retrieved cloud properties from 4STAR sky radiance measurements. Using renewed look-up-table created accounting for actual above cloud AOD values (same as R3 for ORACLES-2016 and R0 for ORACLES-2017).
...
for i,daystr in enumerate(dds):
    ...
    wu.write_ict(hdict,d_dict_out,filepath=fo+'zen_ict_2018/v1/',
...
hs.savemat(fo+'zen_ict_2018/v1/2018_{}_all_cld_retrieved.mat'.format(vr),rtss)
...
rtss = hs.loadmat(fo+'zen_ict_2018/v1/2018_{}_all_cld_retrieved.mat'.format(vr))
...
out_head_RA = []
for d in dds:
    fname_aod = fo+'zen_ict_2018/v1/4STAR-CLD_P3_{}_{vr}.ict'.format(d,vr=vr)
...
for i,d in enumerate(dds):
    ...
    plt.savefig(fo+'zen_ict_2018/v1/{vv}_{}.png'.format(d,vv=vv),dpi=600,transparent=True)
...
hs.savemat(fo+'zen_ict_2018/v1/{}_all_cld_ict_2018.mat'.format(vr),ar)
...
if not 'ar' in locals():
    ar = hs.loadmat(fo+'zen_ict_2018/v1/{}_all_cld_ict_2018.mat'.format(vr))
...
fig = plt.figure()
ax = plt.subplot(111)
m = build_basemap(lower_left=[-90,32],upper_right=[-65,50],ax=ax,larger=False)
...
fig = plt.figure()
ax = plt.subplot(111)
m = build_basemap(lower_left=[-2,-16],upper_right=[15,1],ax=ax,larger=False)
sa = []
for i,daystr in enumerate(dds):
...
cb.set_label('COD')
plt.savefig(fo+'zen_ict_2018/v1/{}_2018_COD_map.png'.format(vr),transparent=True,dpi=600)
...
plt.savefig(fo+'zen_ict_2018/v1/{}_2018_REF_map.png'.format(vr),transparent=True,dpi=600)
...
plt.savefig(fo+'zen_ict_2018/v1/{}_2018_LWP_map.png'.format(vr),transparent=True,dpi=600)
...
ifl = np.where(np.isfinite(ar['tau_fl']))[0][0:-3]
...
plt.figure()
plt.plot(ar['lat_fl'][ifl],ar['tau_fl'][ifl],'.',color='grey',alpha=0.1)
plt.hist2d(ar['lat_fl'][ifl],ar['tau_fl'][ifl],bins=40,normed=True)
plt.xlabel('Latitude [$^\\circ$]')
plt.ylabel('COD')
cb = plt.colorbar()
cb.set_label('Normalized counts')
plt.title('4STAR Cloud optical depth for all ORACLES flights')
plt.savefig(fo+'zen_ict_2018/v1/{}_2018_COD_hist_lat.png'.format(vr),transparent=True,dpi=600)
...
plt.figure()
plt.plot(ar['lon_fl'][ifl],ar['tau_fl'][ifl],'.',color='grey',alpha=0.1)
plt.hist2d(ar['lon_fl'][ifl],ar['tau_fl'][ifl],bins=40,normed=True,range=[[-2,14], [0, 35]])
...
cb.set_label('Normalized counts')
plt.title('4STAR Cloud optical depth for all ORACLES flights')
plt.savefig(fo+'zen_ict_2018/v1/{}_2018_COD_hist_lon.png',transparent=True,dpi=600)
...
plt.figure()
plt.plot(ar['lon_fl'][ifl],ar['ref_fl'][ifl],'.',color='grey',alpha=0.1)
plt.hist2d(ar['lon_fl'][ifl],ar['ref_fl'][ifl],bins=40,normed=True,cmap=plt.cm.gist_earth,range=[[-2,14], [0, 25]])
plt.xlabel('Longitude [$^\\circ$]')
...
plt.savefig(fo+'zen_ict_2018/v1/{}_2018_ref_hist_lon.png',transparent=True,dpi=600)
...
plt.figure()
plt.plot(ar['lat_fl'][ifl],ar['ref_fl'][ifl],'.',color='grey',alpha=0.1)
plt.hist2d(ar['lat_fl'][ifl],ar['ref_fl'][ifl],bins=40,normed=True,cmap=plt.cm.gist_earth,range=[[-16,2], [0, 25]])
plt.ylim(2,25)
plt.xlabel('Latitude [$^\\circ$]')
plt.ylabel('r$_{{eff}}$ [$\\mu$m]')
cb = plt.colorbar()
cb.set_label('Normalized counts')
plt.title('4STAR Effective Radius for all ORACLES flights')
plt.savefig(fo+'zen_ict_2018/v1/{}_ref_hist_lat.png',transparent=True,dpi=600)
...
fig = plt.figure()
plt.hist(ar['tau_fl'][ifl],bins=30,edgecolor='None',color='g',alpha=0.7,normed=True,label='filtered')
...
pu.prelim()
plt.legend(frameon=False)
plt.savefig(fo+'zen_ict_2018/v1/cod_hist_2018.png',transparent=True,dpi=600)


# In[165]:


aam = np.where((ar['utc_fl']<12.0) & np.isfinite(ar['tau_fl'][0:-3]))[0]
apm = np.where((ar['utc_fl']>12.0) & np.isfinite(ar['tau_fl'][0:-3]))[0]
...
fig = plt.figure()
plt.hist(ar['tau_fl'][ifl],bins=30,edgecolor='None',color='g',alpha=0.3,normed=True,label='filtered')
plt.hist(ar['tau'],bins=30,edgecolor='None',color='b',alpha=0.1,normed=True,range=(0,70),label='All points')
...
pu.prelim()
plt.legend(frameon=False)
plt.savefig(fo+'zen_ict_2018/v1/{}_2018_cod_hist_pm_am.png'.format(vr),transparent=True,dpi=600)
...
#pu.prelim()
plt.legend(frameon=False)
plt.savefig(fo+'zen_ict_2018/v1/{}_2018_cod_hist_pm_am.png',transparent=True,dpi=600)
...
fig = plt.figure()
plt.hist(ar['ref_fl'][ifl],bins=30,edgecolor='None',color='grey',alpha=0.7,normed=True,label='filtered')
...
#pu.prelim()
plt.legend(frameon=False)
plt.savefig(fo+'zen_ict_2018/v1/{}_2018_ref_hist.png'.format(vr),transparent=True,dpi=600)
...
fig,ax = plt.subplots(2,1)
ax = ax.ravel()
ax[0].hist(ar['tau_fl'][ifl],bins=30,edgecolor='None',color='g',alpha=0.7,normed=True,label='filtered')
ax[0].hist(ar['tau'],bins=30,edgecolor='None',color='b',alpha=0.1,normed=True,range=(0,70),label='all points')
ax[0].set_ylabel('Normed counts')
ax[0].set_xlabel('COD')
ax[0].grid()
...
ax[1].hist(ar['ref_fl'][ifl],bins=30,edgecolor='None',color='grey',alpha=0.7,normed=True,label='filtered')
ax[1].hist(ar['ref'],bins=30,edgecolor='None',color='b',alpha=0.1,normed=True,range=(0,30),label='all points')
ax[1].set_ylabel('Normed counts')
ax[1].set_xlabel('r$_{{eff}}$ [$\\mu$m]')
plt.grid()
...
plt.savefig(fo+'zen_ict_2018/v1/{}_2018__ref_cod_hist.png'.format(vr),transparent=True,dpi=600)
